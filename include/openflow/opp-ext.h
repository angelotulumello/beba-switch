...
#define OPP_VENDOR_ID 0xBEBABEBA
...
struct ofp_opp_instruction_experimenter_header {
...
OFP_ASSERT(sizeof(struct ofp_opp_instruction_experimenter_header) == 16);

struct ofp_exp_instruction_in_switch_pkt_gen {
	struct ofp_opp_instruction_experimenter_header header; /* OpenFlow standard experimenter instruction header */
	uint32_t pkttmp_id;
	uint8_t pad[4];
...
struct ofp_opp_action_experimenter_header {
...
OFP_ASSERT(sizeof(struct ofp_opp_action_experimenter_header) == 16);

/* Action structure for OFPAT_EXP_SET_STATE */
struct ofp_exp_action_set_state {
    struct ofp_opp_action_experimenter_header header;
