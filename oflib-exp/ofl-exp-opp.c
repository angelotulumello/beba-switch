...
int
ofl_exp_opp_msg_pack(struct ofl_msg_experimenter const *msg, uint8_t **buf, size_t *buf_len, struct ofl_exp const *exp UNUSED)
{
    struct ofl_exp_opp_msg_header *exp_msg = (struct ofl_exp_opp_msg_header *)msg;
    switch (exp_msg->type) {
       ...
       case(OFPT_EXP_STATE_CHANGED): {
           ...
           *buf     = (uint8_t *)malloc(*buf_len);
           ...
           ntf_msg->header.experimenter = htonl(OPP_VENDOR_ID);
        ...
        default: {
            OFL_LOG_WARN(LOG_MODULE, "Trying to pack unknown Opp Experimenter message.");
            return -1;
        }
    }
}
...
ofl_err
ofl_exp_opp_msg_unpack(struct ofp_header const *oh, size_t *len, struct ofl_msg_experimenter **msg, struct ofl_exp const *exp)
{
    ...
    switch (ntohl(exp_header->exp_type)) {
        ...
        default: {
            ...
            (*msg) = dm;
            OFL_LOG_WARN(LOG_MODULE, "Trying to unpack unknown Opp Experimenter message.");
...
ofl_exp_opp_msg_free(struct ofl_msg_experimenter *msg)
{
    ...
    switch (exp->type) {
        case (OFPT_EXP_STATE_MOD):
        {
            ...
            OFL_LOG_DBG(LOG_MODULE, "Free Opp STATE_MOD Experimenter message. oppexp{type=\"%u\", command=\"%u\"}", exp->type, state_mod->command);
            free(msg);
            break;
        }
        case (OFPT_EXP_PKTTMP_MOD):
        {
            ...
            OFL_LOG_DBG(LOG_MODULE, "Free Opp PKTTMP_MOD Experimenter message. oppexp{type=\"%u\", command=\"%u\"}", exp->type, pkttmp_mod->command);
            free(msg);
            break;
        }
        case (OFPT_EXP_STATE_CHANGED):
        {
            OFL_LOG_DBG(LOG_MODULE, "Free Opp OFPT_EXP_STATE_CHANGED Experimenter message. oppexp{type=\"%u\"}", exp->type);
            free(msg);
            break;
        }
        case (OFPT_EXP_FLOW_NOTIFICATION):
        {
            ...
            OFL_LOG_DBG(LOG_MODULE, "Free Opp FLOW_NOTIFICATION Experimenter message. oppexp{type=\"%u\", table_id=\"%u\"}", exp->type, msg->table_id);
        ...
        default: {
            OFL_LOG_WARN(LOG_MODULE, "Trying to free unknown Opp Experimenter message.");
        }
    }
    return 0;
}
...
char *
ofl_exp_opp_msg_to_string(struct ofl_msg_experimenter const *msg, struct ofl_exp const *exp)
{
    char *str;
    size_t str_size;
    ...
    switch (exp_msg->type) {
        case (OFPT_EXP_STATE_MOD):
        {
            ...
            OFL_LOG_DBG(LOG_MODULE, "Print Opp STATE_MOD Experimenter message OPP_MSG{type=\"%u\", command=\"%u\"}", exp_msg->type, state_mod->command);
            break;
        }
        case (OFPT_EXP_PKTTMP_MOD):
        {
            struct ofl_exp_msg_pkttmp_mod *pkttmp_mod = (struct ofl_exp_msg_pkttmp_mod *)exp_msg;
            OFL_LOG_DBG(LOG_MODULE, "Print Opp PKTTMP_MOD Experimenter message OPP_MSG{type=\"%u\", command=\"%u\"}", exp_msg->type, pkttmp_mod->command);
            break;
        }
        case (OFPT_EXP_STATE_CHANGED):
        {
            OFL_LOG_DBG(LOG_MODULE, "Print Opp OFPT_EXP_STATE_CHANGED Experimenter message OPP_MSG{type=\"%u\"}", exp_msg->type);
            break;
        }
        case (OFPT_EXP_FLOW_NOTIFICATION):{
        ...
        default: {
            OFL_LOG_WARN(LOG_MODULE, "Trying to print unknown Opp Experimenter message UNKN_OPP_MSG{type=\"%u\"}", exp_msg->type);
            break;
        }
    }
    fclose(stream);
...
ofl_err
ofl_exp_opp_act_unpack(struct ofp_action_header const *src, size_t *len, struct ofl_action_header **dst)
{
    struct ofp_action_experimenter_header const *exp;
    struct ofp_opp_action_experimenter_header const *ext;
    int i=0;
    ofl_err error;
    ...
    ext = (struct ofp_opp_action_experimenter_header const *)exp;
    ...
    switch (ntohl(ext->act_type)) {
        ...
        default:
        {
            ...
            (*dst) = (struct ofl_action_header *)da;
            OFL_LOG_WARN(LOG_MODULE, "Trying to unpack unknown Opp Experimenter action.");
...
int
ofl_exp_opp_act_pack(struct ofl_action_header const *src, struct ofp_action_header *dst)
{

    struct ofl_action_experimenter *exp = (struct ofl_action_experimenter *) src;
    struct ofl_exp_opp_act_header *ext = (struct ofl_exp_opp_act_header *) exp;
    int i=0;
...
size_t
ofl_exp_opp_act_ofp_len(struct ofl_action_header const *act)
{
...
char *
ofl_exp_opp_act_to_string(struct ofl_action_header const *act)
{
    struct ofl_action_experimenter *exp = (struct ofl_action_experimenter *) act;
    struct ofl_exp_opp_act_header *ext = (struct ofl_exp_opp_act_header *) exp;
...
int
ofl_exp_opp_act_free(struct ofl_action_header *act) {
    struct ofl_action_experimenter *exp = (struct ofl_action_experimenter *) act;
    struct ofl_exp_opp_act_header *ext = (struct ofl_exp_opp_act_header *) exp;
    switch (ext->act_type) {
        case (OFPAT_EXP_SET_STATE): {
        ...
        default: {
            OFL_LOG_WARN(LOG_MODULE, "Trying to free unknown Opp Experimenter action.");
        }
    }
        return 0;
}
...
int
ofl_exp_opp_stats_req_pack(struct ofl_msg_multipart_request_experimenter const *ext, uint8_t **buf,
                            size_t *buf_len, struct ofl_exp const *exp) {
    struct ofl_exp_opp_msg_multipart_request *e = (struct ofl_exp_opp_msg_multipart_request *) ext;
    switch (e->type) {
        ...
        case (OFPMP_EXP_STATE_STATS): {
            ...
            *buf = (uint8_t *) malloc(*buf_len);
            ...
            exp_header->experimenter = htonl(OPP_VENDOR_ID);
...
int
ofl_exp_opp_stats_reply_pack(struct ofl_msg_multipart_reply_experimenter const *ext, uint8_t **buf,
                              size_t *buf_len, struct ofl_exp const *exp) {
    struct ofl_exp_opp_msg_multipart_reply *e = (struct ofl_exp_opp_msg_multipart_reply *) ext;
    switch (e->type) {
        case (OFPMP_EXP_STATE_STATS_AND_DELETE):
        case (OFPMP_EXP_STATE_STATS): {
            ...
            *buf = (uint8_t *) malloc(*buf_len);
            ...
            ext_header->experimenter = htonl(OPP_VENDOR_ID);
...
ofl_err
ofl_exp_opp_stats_req_unpack(struct ofp_multipart_request const *os, uint8_t const *buf, size_t *len,
...
ofl_err
ofl_exp_opp_stats_reply_unpack(struct ofp_multipart_reply const *os, uint8_t const *buf, size_t *len,
...
char *
ofl_exp_opp_stats_request_to_string(struct ofl_msg_multipart_request_experimenter const *ext,
                                     struct ofl_exp const *exp) {
    struct ofl_exp_opp_msg_multipart_request const *e = (struct ofl_exp_opp_msg_multipart_request const *) ext;
    char *str;
    size_t str_size;
    FILE *stream = open_memstream(&str, &str_size);
    switch (e->type) {
...
char *
ofl_exp_opp_stats_reply_to_string(struct ofl_msg_multipart_reply_experimenter const *ext,
                                   struct ofl_exp const *exp) {
    struct ofl_exp_opp_msg_multipart_reply *e = (struct ofl_exp_opp_msg_multipart_reply *) ext;
    char *str;
    size_t str_size;
...
int
ofl_exp_opp_stats_req_free(struct ofl_msg_multipart_request_header *msg) {
    struct ofl_msg_multipart_request_experimenter *exp = (struct ofl_msg_multipart_request_experimenter *) msg;
    struct ofl_exp_opp_msg_multipart_request *ext = (struct ofl_exp_opp_msg_multipart_request *) exp;
    switch (ext->type) {
        case (OFPMP_EXP_STATE_STATS_AND_DELETE):
        case (OFPMP_EXP_STATE_STATS): {
...
int
ofl_exp_opp_stats_reply_free(struct ofl_msg_multipart_reply_header *msg) {
    struct ofl_msg_multipart_reply_experimenter *exp = (struct ofl_msg_multipart_reply_experimenter *) msg;
    struct ofl_exp_opp_msg_multipart_reply *ext = (struct ofl_exp_opp_msg_multipart_reply *) exp;
    int i;
    switch (ext->type) {
        case (OFPMP_EXP_STATE_STATS_AND_DELETE):
        case (OFPMP_EXP_STATE_STATS): {
...
int
ofl_exp_opp_field_unpack(struct ofl_match *match, struct oxm_field const *f, void const *experimenter_id,
...
void
ofl_exp_opp_field_pack(struct ofpbuf *buf, struct ofl_match_tlv const *oft) {
...
void
ofl_exp_opp_field_match(struct ofl_match_tlv *f, int *packet_header, int *field_len, uint8_t **flow_val,
                         uint8_t **flow_mask) {
...
void
ofl_exp_opp_field_compare(struct ofl_match_tlv *packet_f, uint8_t **packet_val) {
    *packet_val = packet_f->value + EXP_ID_LEN;
}

void
ofl_exp_opp_field_match_std(struct ofl_match_tlv *flow_mod_match, struct ofl_match_tlv *flow_entry_match UNUSED,
...
void
ofl_exp_opp_field_overlap_a(struct ofl_match_tlv *f_a, int *field_len, uint8_t **val_a, uint8_t **mask_a,
...
void
ofl_exp_opp_field_overlap_b(struct ofl_match_tlv *f_b, int *field_len, uint8_t **val_b, uint8_t **mask_b,
                             uint64_t *all_mask) {
...
void
ofl_exp_opp_error_pack(struct ofl_msg_exp_error const *msg, uint8_t **buf, size_t *buf_len) {
...
void
ofl_exp_opp_error_free(struct ofl_msg_exp_error *msg) {
    free(msg->data);
    free(msg);
}

char *
ofl_exp_opp_error_to_string(struct ofl_msg_exp_error const *msg) {
    char *str;
    size_t str_size;
    FILE *stream = open_memstream(&str, &str_size);
    fprintf(stream, "{type=\"");
    ...
    fprintf(stream, "\", exp_type=\"");
    ofl_error_opp_exp_type_print(stream, msg->exp_type);
...
int
ofl_exp_opp_inst_pack(struct ofl_instruction_header const *src, struct ofp_instruction *dst) {

    struct ofl_instruction_experimenter *exp = (struct ofl_instruction_experimenter *) src;
    struct ofl_exp_opp_instr_header *ext = (struct ofl_exp_opp_instr_header *) exp;
    ...
    switch (ext->instr_type) {
        case OFPIT_IN_SWITCH_PKT_GEN: {
            ...
            OFL_LOG_DBG(LOG_MODULE, "ofl_exp_opp_inst_pack OFPIT_IN_SWITCH_PKT_GEN");
            ...
            di->header.header.experimenter = htonl(exp->experimenter_id); //OPP_VENDOR_ID
...
ofl_err
ofl_exp_opp_inst_unpack(struct ofp_instruction const *src, size_t *len, struct ofl_instruction_header **dst) {

    struct ofl_instruction_header *inst = NULL;
    size_t ilen;
    ofl_err error = 0;
    struct ofp_instruction_experimenter_header *exp;
    struct ofp_opp_instruction_experimenter_header *opp_exp;

    OFL_LOG_DBG(LOG_MODULE, "ofl_exp_opp_inst_unpack");
    ...
    opp_exp = (struct ofp_opp_instruction_experimenter_header *) exp;
    switch (ntohl(opp_exp->instr_type)) {
        case OFPIT_IN_SWITCH_PKT_GEN: {
            ...
            di->header.header.experimenter_id = ntohl(exp->experimenter); //OPP_VENDOR_ID
            ...
            di->header.instr_type = ntohl(opp_exp->instr_type); //OFPIT_IN_SWITCH_PKT_GEN
        ...
        default: {
            ...
            di->experimenter_id = ntohl(exp->experimenter); //OPP_VENDOR_ID
            inst = (struct ofl_instruction_header *) di;
            OFL_LOG_WARN(LOG_MODULE, "The received OPP instruction type (%u) is invalid.",
                         ntohs(opp_exp->instr_type));
...
int
ofl_exp_opp_inst_free(struct ofl_instruction_header *i) {
    ...
    switch (ext->instr_type) {
        case (OFPIT_IN_SWITCH_PKT_GEN): {
            OFL_LOG_DBG(LOG_MODULE, "Freeing OPP instruction IN_SWITCH_PKT_GEN.");
        ...
        default: {
            OFL_LOG_WARN(LOG_MODULE, "Unknown OPP instruction type. Perhaps not freed correctly");
        }
    }
    free(i);
    return 1;
...
size_t
ofl_exp_opp_inst_ofp_len(struct ofl_instruction_header const *i) {
    struct ofl_instruction_experimenter *exp = (struct ofl_instruction_experimenter *) i;
    ...
    switch (ext->instr_type) {
        case OFPIT_IN_SWITCH_PKT_GEN: {
            ...
            OFL_LOG_DBG(LOG_MODULE, "ofl_exp_opp_inst_ofp_len");
            // TODO We may need to pass the ofl_exp callbacks instead of NULL
//              return sizeof(struct ofl_exp_opp_instr_header)
//                      + ofl_actions_ofp_total_len(i->actions, i->actions_num, exp);
        ...
        default:
            OFL_LOG_WARN(LOG_MODULE, "Trying to len unknown OPP instruction type.");
            return 0;
    }
}

char *
ofl_exp_opp_inst_to_string(struct ofl_instruction_header const *i) {
    struct ofl_instruction_experimenter *exp = (struct ofl_instruction_experimenter *) i;
    ...
    switch (ext->instr_type) {
        case (OFPIT_IN_SWITCH_PKT_GEN): {
            OFL_LOG_DBG(LOG_MODULE, "Trying to print OPP Experimenter instruction. Not implemented yet!");
            fprintf(stream, "OFPIT{type=\"%u\"}", ext->instr_type);
            break;
        }
        default: {
            OFL_LOG_WARN(LOG_MODULE, "Trying to print unknown OPP Experimenter instruction.");
            fprintf(stream, "OFPIT{type=\"%u\"}", ext->instr_type);
        }
    }
